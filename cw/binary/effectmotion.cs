...
class EffectMotion : base.CWBinaryBase {
    ...
    // """;
    public EffectMotion(UNK parent, UNK f, bool yadodata=false, UNK dataversion=4) :  base(parent, f, yadodata) {
        this.tabtype = f.byte();
        ...
        // 召喚(BeastCardインスタンスを生成)
            ...
            beasts_num = f.dword();
            this.beasts = [beast.BeastCard(self, f, summoneffect=true) for _cnt in xrange(beasts_num)];
        } else {
            throw ValueError(this.fpath);
        }
