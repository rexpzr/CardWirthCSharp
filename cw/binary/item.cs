...
class ItemCard(base.CWBinaryBase):
    ...
    public UNK __init__(parent, f, yadodata=false, nameonly=false, materialdir="Material", image_export=true) {
        ...
        this.price = f.dword();
        this.enhance_avoid2 = f.dword();
        this.enhance_resist2 = f.dword();
        this.enhance_defense2 = f.dword();
    ...
    public UNK get_data() {
        if (this.data == null) {
            ...
            prop.append(e);
            e = cw.data.make_element("Price", str(this.price));
            prop.append(e);
            e = cw.data.make_element("EnhanceOwner");
            e.set("avoid", str(this.enhance_avoid2));
            e.set("resist", str(this.enhance_resist2));
            e.set("defense", str(this.enhance_defense2));
            prop.append(e);
        ...
        foreach (var e in data) {
            if (e.tag == "Property") {
                foreach (var prop in e) {
                    ...
                    } else if (prop.tag == "Price") {
                        price = int(prop.text);
                    } else if (prop.tag == "EnhanceOwner") {
